...
const MenuNavbar = ({ show, setShow }: { show: {category: string; value: boolean }; setShow: React.Dispatch<React.SetStateAction<{ category: string; value: boolean }>> }) => {
  const hanndleHover = () => {
  ...
  return (
    <div className="relative cursor-pointer" onMouseOver={hanndleHover} onMouseLeave={hanndleHoverLeave}>
      ...
      <IconChevronCompactUp className={show.category === "movies" ? "w-4 h-4 transition-transform duration-200" :  "w-4 h-4 rotate-180 "} />
      </div>
